/**
 * Sample Skeleton for 'MainViewRockwell.fxml' Controller Class
 */
...
public class MainViewControllerRockwell extends ViewController implements Initializable {
	...
	static ModelRockwell model;
	private Settings properties;
	static List<File> selectedFiles_L5X;
	static List<File> selectedFiles_Xlsx;
	...
	void openChooseFileXls(ActionEvent event) {
		...
		try {
			...
			ModelRockwell.logRock.debug("IOException");
		}
		ModelRockwell.logRock.debug("Rockwell settings changes saved ");
...
//		if (mbPrefixDriver.getText().equals("---") || mbPrefixDriver.getText().equals("Prefix Driver"))
//			return txtFieldPrefixDriver.getText();
//		return mbPrefixDriver.getText();
		...
		protected Void call() throws Exception {
			...
			if (selectedFiles_L5X != null) {
				if (!rawExport) {
					for (File f : selectedFiles_L5X) {
						model.clearLists();
						ModelRockwell.logRock.info("Apertura elenco file " + f.getAbsolutePath());
						model.readL5X(f.getAbsolutePath());
						ModelRockwell.logRock.info("Generazione file xlsx");
...
//			selectedFiles.clear();
	...
	public class Convert_Xlsx_to_Csv extends Task<Void> {
		...
		protected Void call() throws Exception {
			if (selectedFiles_Xlsx != null) {
				for (File f : selectedFiles_Xlsx) {
					ModelRockwell.logRock.info("Apertura file .xls " + f.getAbsolutePath());
					if (isFileClosed(f)) {
...
//		try {
//			testOut = new FileOutputStream(file.getAbsolutePath());
//			testOut.flush();
//			testOut.close();
//			testOut.flush();
