...
public class ItemLWord extends Item {

	private int value;

	public ItemLWord(String dbName, String stringName, String stringComment, Address addressGlobal, ItemStruct _parent) {
		this.dbName = dbName;
		this.value = 0;
	...
	public String toStringExtended() {
		if (comment.equals(""))
			return "ItemLWord [value=" + value + ", name=" + name + ", address=" + address + ", simbolicName="
					+ this.getSimbolicName() + "]";
		return "ItemLWord [value=" + value + ", name=" + name + ", comment=" + comment + ", address=" + address
				+ ", simbolicName=" + this.getSimbolicName() + "]";
	}

	@Override
	public Item clone() throws CloneNotSupportedException {
		return new ItemLWord(this.getDbName(), this.getName(), this.getComment(), this.getAddress().clone(),
				this.getParent());
	}
	...
	public static Item makeItemFromString(ItemStruct workingStruct, String str, boolean typeChanged) {
		...
		ItemLWord itemLWord = new ItemLWord(workingStruct.getDbName(), ModelSiemens.getNameString(str), comment,
...
//		ModelSiemens.logSiem.info(itemLWord.toStringExtended());
...
//		ModelSiemens.logSiem.info("LTIME this: "+this.address.gByte()+"  +  global: "+gAddr.gByte());
	...
	protected void insertItem(Item item, Row rowGen) {
		String strFormula = item.getDbName() + Item.getStringTypeForSCADATag(item)+"_DB" + ItemStruct.intToStringFormatted(item.getAddress().getDB()) + "LWORD"
		...
		if (item.getSimbolicName().toString().contains(".R."))
			rowGen.createCell(6).setCellValue("Int_read<AI_INT>");// TODO questo è un LWord ma per il momento viene
																	// trattato come Int
		if (item.getSimbolicName().toString().contains(".W."))
			rowGen.createCell(6).setCellValue("Int_write<WAI_INT>");// TODO questo è un LWord ma per il momento viene
																	// trattato come Int
	}
}
