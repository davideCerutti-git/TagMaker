...
public class ItemLTime extends Item {

	private int value;

	public ItemLTime(String dbName, String stringName, String stringComment, Address addressGlobal, ItemStruct _parent) {
		this.dbName = dbName;
		this.value = 0;
	...
	public String toStringExtended() {
		if (comment.equals(""))
			return "ItemLTime [value=" + value + ", name=" + name + ", address=" + address + ", simbolicName="
					+ this.getSimbolicName() + "]";
		return "ItemLTime [value=" + value + ", name=" + name + ", comment=" + comment + ", address=" + address
				+ ", simbolicName=" + this.getSimbolicName() + "]";
	}

	@Override
	public Item clone() throws CloneNotSupportedException {
		return new ItemLTime(this.getDbName(), this.getName(), this.getComment(), this.getAddress().clone(),
				this.getParent());
	}
...
//		ModelSiemens.logSiem.info("LTIME this: "+this.address.gByte()+"  +  global: "+gAddr.gByte());
	...
	protected void insertItem(Item item, Row rowGen) {
		String strFormula = item.getDbName() + "_DB" + ItemStruct.intToStringFormatted(item.getAddress().getDB()) + "LTIME"
		...
		if (item.getSimbolicName().toString().contains(".R."))
			rowGen.createCell(6).setCellValue("Int_read<AI_INT>");// TODO questo è un LTime ma per il momento viene
																	// trattato come Int
