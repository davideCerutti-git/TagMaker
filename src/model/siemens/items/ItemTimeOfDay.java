...
public class ItemTimeOfDay extends Item {
	...
	public ItemTimeOfDay(String dbName, String stringName, String stringComment, Address addressGlobal,
			ItemStruct _parent) {
		this.dbName = dbName;
		this.value = 0;
	...
	public String toStringExtended() {
		if (comment.equals(""))
			return "ItemTimeOfDay [value=" + value + ", name=" + name + ", address=" + address + ", simbolicName="
	...
	public Item clone() throws CloneNotSupportedException {
		return new ItemTimeOfDay(this.getDbName(), this.getName(), this.getComment(), this.getAddress().clone(),
				this.getParent());
	}
	...
	public static Item makeItemFromString(ItemStruct workingStruct, String str, boolean typeChanged) {
		...
		ItemTimeOfDay itemTimeOfDay = new ItemTimeOfDay(workingStruct.getDbName(), str.split(":")[0].trim(), comment,
	...
	protected void insertItem(Item item, Row rowGen) {
		String strFormula = item.getDbName() + Item.getStringTypeForSCADATag(item) + "_DB"
				+ ItemStruct.intToStringFormatted(item.getAddress().getDB()) + "TIMEOFDAY"
