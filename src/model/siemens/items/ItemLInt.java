...
//		ModelSiemens.logSiem.info("LINT this: "+this.address.gByte()+"  +  global: "+gAddr.gByte());
	...
	protected void insertItem(Item item, Row rowGen) {
		...
		if (item.getSimbolicName().toString().contains(".R."))
			rowGen.createCell(6).setCellValue("Int_read<AI_INT>");// TODO questo è un LInt ma per il momento viene
																	// trattato come Int
		if (item.getSimbolicName().toString().contains(".W."))
			rowGen.createCell(6).setCellValue("Int_write<WAI_INT>");// TODO questo è un LInt ma per il momento viene
																	// trattato come Int
	}
}
